...
def get_yaml_params(params_file: Path, section: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            f'Required section sequence [{".".join(sections)}] not present in parameters file -> {params_file}'
        )
...
def get_params_w_overrides(
    *, params: Path, section: str, override_params: Path, override_section: str = None
) -> dict:
